...
export class R15TelaInicialAdministrador implements OnInit {
  ...
  readonly filtros = signal<FiltroOpcao[]>([
    ...
    { id: 'altoSaldo', label: 'Alto Saldo Positivo', selecionado: true },
    { id: 'emAlerta', label: 'Em Alerta (Saldo Negativo)', selecionado: true },
  ]);
  ...
  get gerentesFiltrados(): GerenteDashboard[] {
    ...
    gerentesFiltrados = gerentesFiltrados.filter((gerente) => {
      ...
      const temAltoSaldo = gerente.totalSaldoPositivo > 100000;
      const temAlerta = gerente.totalSaldoNegativo > 0;
      ...
      const passouFiltros =
        ...
        (temAltoSaldo && filtrosPorId['altoSaldo']) ||
        (temAlerta && filtrosPorId['emAlerta']);
